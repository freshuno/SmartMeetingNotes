...
openai.api_key = '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'
...
class MeetingRecorderApp:
    ...
    def summarize_notes(self):
        if not self.transcription.strip():
            messagebox.showinfo("Summarize Notes", "No transcription available to summarize!")
            return

        try:
            response = openai.chat.completions.create(
                model="gpt-3.5-turbo",  # Możesz użyć modelu 4, jeśli jest dostępny
                messages=[
            ...
            if not os.path.exists(notes_folder):
                os.makedirs(notes_folder)
            timestamp = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
            summary_filename = os.path.join(notes_folder, f"summary_{timestamp}.txt")
            with open(summary_filename, "w", encoding="utf-8") as f:
                f.write(summary)

            messagebox.showinfo("Summary", f"Summary saved as:\n- {summary_filename}")
        except Exception as e:
            messagebox.showerror("Error", f"Failed to summarize notes: {str(e)}")
